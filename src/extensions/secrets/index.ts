...
export default function secretsStoreExtension(pi: ExtensionAPI) {
  ...
  const agentDir = process.env.PI_AGENT_DIR || process.env.PI_CODING_AGENT_DIR || extensionDir;
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, rawParams, _onUpdate, _ctx, signal) {
      ...
      if (execResult.code !== 0) {
        ...
        return {
          content: [{ type: "text", text: `secrets_store failed: ${err}` }],
          details: { ok: false, exitCode: execResult.code },
        };
      }
